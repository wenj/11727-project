...
def process_file(fname):
    ...
    d = json.load(open(f"results_notbkup/results/{fname}.json"))

    for i, chain in enumerate(d):
        if len(chain) <= 1:
            continue
        for mention in chain:
            ms = mention['sentNum']
            mc = mention['headIndex']
            mb = mention['startIndex']
            me = mention['endIndex']
            me = mb + len(mention['mention'].strip().split())
            s1 = mention['mention'].strip()
            s2 = ' '.join([pred_doc[ms-1][i][0] for i in range(mb-1, me-1)]).strip()
            if s1 != s2:
                print(s1, "\t", s2)
#            print(ms, mc, mb, me)
#            print(pred_doc[ms-1])
            if mb + 1 == me:
                pred_doc[ms-1][mb-1][1].append(f"({i})")
            else:
                pred_doc[ms-1][mb-1][1].append(f"({i}")
                pred_doc[ms-1][me-2][1].append(f"{i})")
...
#                print(subs(pred_doc, p_begin, p_end))
    ...
    print(len(doc))
    print(len(pred_doc))
    print_to_file("model.key", doc, d_name)
    print_to_file("model.pred", pred_doc, d_name)
